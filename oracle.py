...
def get_random_plugboard(min_count: int, max_count: int, rng: random.Random):
    ...
    pairs = [alphabet[2 * i] + alphabet[2 * i + 1] for i in range(len(ALPHABET) // 2)]
    count = rng.randint(min_count, max_count)
    plugboard = get_permutation_from_cycles(pairs[:count])
    return plugboard
...
def get_random_settings(rng: random.Random):
    ...
    ring_settings = [rng.randint(0, len(ALPHABET) - 1) for _ in range(3)]
    reflector = rng.choice(list(ENIGMA1_REFLECTORS.values()))
    rotors = list(ENIGMA1_ROTORS.values())
    rng.shuffle(rotors)
    rotors = rotors[:3]
    plugboard = get_random_plugboard(4, 6, rng=rng)
